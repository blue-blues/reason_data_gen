#!/bin/bash
# Script to run the fixed distributed pipeline on Linux

echo "==================================================="
echo "Mathematical Reasoning Pipeline - Fixed Distributed Mode"
echo "==================================================="
echo ""
echo "This script runs the fixed distributed pipeline that avoids the DTensor error."
echo ""
...
# Run the fixed distributed pipeline
python run_distributed_fixed.py --dataset numina_math --force-download --batch-size 4 --optimize-gpu --no-warnings
if [ $? -ne 0 ]; then
    echo "Error running fixed distributed pipeline"
    exit 1
fi
